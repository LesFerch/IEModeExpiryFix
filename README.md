...
You can download the script in **ZIP** format via the green **Code** button or download it directlly as a **VBS** file by clicking on the [script link](https://github.com/LesFerch/IEModeExpiryFix/blob/main/IEModeExpiryFix.vbs) and then either **Alt-Click** the **Raw** button (disabled in some browsers) or **Right-Click** the **Raw** button and select **Save link as...**.
